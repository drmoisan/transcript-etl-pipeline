...
2. **tkinter Dependency**: UI dialogs require tkinter. Workaround: Provide all CLI arguments to bypass UI.

3. **Input Format Dependency**: Best results require properly formatted transcript input with clear speaker labels and metadata.
...
- Review policy files (code-change.instructions.md, unit-test-policy.md, developer-tooling.md)
- Document the plan if part of multi-step work
...
- More sophisticated name-matching (fuzzy matching, nickname support)
- Machine learning-based speaker identification
- Persistent speaker mapping across sessions

### 🟡 Additional Output Formats (0%)
**Effort**: Medium  
**Priority**: Low

- PDF generation (via reportlab or fpdf)
