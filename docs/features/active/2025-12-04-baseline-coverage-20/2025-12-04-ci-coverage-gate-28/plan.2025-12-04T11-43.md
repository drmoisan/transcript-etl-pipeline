...
- [x] [P0-T1] Read `.github/copilot-instructions.md` in the repo root to establish baseline agent rules (line references not applicable).
  - Acceptance: `powershell -Command "Test-Path .github/copilot-instructions.md"` exits with code 0.
- [x] [P0-T2] Read `.github/instructions/general-code-change.instructions.md` to confirm the required workflow (line references not applicable).
  - Acceptance: `powershell -Command "Test-Path .github/instructions/general-code-change.instructions.md"` exits with code 0.
- [x] [P0-T3] Read `.github/instructions/general-unit-test.instructions.md` to confirm unit test rules (line references not applicable).
  - Acceptance: `powershell -Command "Test-Path .github/instructions/general-unit-test.instructions.md"` exits with code 0.
- [x] [P0-T4] Read `.github/instructions/python-code-change.instructions.md` to confirm Python-specific requirements (line references not applicable).
  - Acceptance: `powershell -Command "Test-Path .github/instructions/python-code-change.instructions.md"` exits with code 0.
- [x] [P0-T5] Read `.github/instructions/python-unit-test.instructions.md` to confirm Python testing standards (line references not applicable).
  - Acceptance: `powershell -Command "Test-Path .github/instructions/python-unit-test.instructions.md"` exits with code 0.
- [x] [P0-T6] Read `.github/instructions/github-actions.instructions.md` before editing `.github/workflows/ci.yml` (line references not applicable).
  - Acceptance: `powershell -Command "Test-Path .github/instructions/github-actions.instructions.md"` exits with code 0.
- [x] [P0-T7] Read `.github/instructions/github-actions-ci-cd-best-practices.instructions.md` before editing `.github/workflows/ci.yml` (line references not applicable).
...
- [x] [P0-T8] Capture baseline formatter output for Python using `poetry run black .` from repo root.
  - Acceptance: Evidence saved to `docs/features/active/2025-12-04-baseline-coverage-20/2025-12-04-ci-coverage-gate-28/evidence/baseline/black.<yyyy-MM-ddTHH-mm>.md` with `Timestamp` (format `yyyy-MM-ddTHH-mm`), `Command`, and `EXIT_CODE` fields; command exits with code 0.
- [x] [P0-T9] Capture baseline lint output for Python using `poetry run ruff check` from repo root.
  - Acceptance: Evidence saved to `docs/features/active/2025-12-04-baseline-coverage-20/2025-12-04-ci-coverage-gate-28/evidence/baseline/ruff.<yyyy-MM-ddTHH-mm>.md` with `Timestamp` (format `yyyy-MM-ddTHH-mm`), `Command`, and `EXIT_CODE` fields; command exits with code 0.
- [x] [P0-T10] Capture baseline type-check output for Python using `poetry run pyright` from repo root.
  - Acceptance: Evidence saved to `docs/features/active/2025-12-04-baseline-coverage-20/2025-12-04-ci-coverage-gate-28/evidence/baseline/pyright.<yyyy-MM-ddTHH-mm>.md` with `Timestamp` (format `yyyy-MM-ddTHH-mm`), `Command`, and `EXIT_CODE` fields; command exits with code 0.
- [x] [P0-T11] Capture baseline test output for Python using `poetry run pytest --cov=src/transcript_etl_pipeline --cov=scripts/dev_tools --cov-report=term-missing` from repo root.
  - Acceptance: Evidence saved to `docs/features/active/2025-12-04-baseline-coverage-20/2025-12-04-ci-coverage-gate-28/evidence/baseline/pytest.<yyyy-MM-ddTHH-mm>.md` with `Timestamp` (format `yyyy-MM-ddTHH-mm`), `Command`, and `EXIT_CODE` fields; command exits with code 0.
- [x] [P0-T12] Capture baseline actionlint output using `pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File scripts/dev-tools/run-actionlint.ps1` from repo root.
  - Acceptance: Evidence saved to `docs/features/active/2025-12-04-baseline-coverage-20/2025-12-04-ci-coverage-gate-28/evidence/baseline/actionlint.<yyyy-MM-ddTHH-mm>.md` with `Timestamp` (format `yyyy-MM-ddTHH-mm`), `Command`, and `EXIT_CODE` fields; command exits with code 0.
...
- [x] [P3-T1] Update `docs/features/active/2025-12-04-baseline-coverage-20/2025-12-04-ci-coverage-gate-28/28-ci-coverage-gate.md` to include the final `fail_under = 15` setting and the threshold ratchet plan table exactly as specified in the Implementation Notes section (REQ-4).
...
- [x] [P3-T3] Create the issue update mirror evidence artifact for Issue #28 at `docs/features/active/2025-12-04-baseline-coverage-20/2025-12-04-ci-coverage-gate-28/evidence/issue-updates/issue-28.<yyyy-MM-ddTHH-mm>.md` with the exact update text, `Timestamp` (format `yyyy-MM-ddTHH-mm`), and `PostedAs` (`body` or `comment`). If posted as a comment, include the comment URL; if posted as body update, include the issue URL and `IssueUpdatedAt`. If `PostedAs: body`, mirror the same update into the local `issue.md` in the current feature scope (or feature root if no versioned issue file exists).
  - Acceptance: Artifact exists at the specified path with required fields and mirrored `issue.md` update when applicable.

### Phase 4 — QA (Python Toolchain)
- [x] [P4-T1] Run `poetry run black .` and confirm the formatter exits with code 0; if it modifies files or fails, repeat after fixing and restart from [P4-T1].
...
- [x] [P4-T3] Run `poetry run pyright` and confirm type checking exits with code 0; if it fails, fix issues and restart from [P4-T1].
  - Acceptance: Command exits with code 0.
- [x] [P4-T4] Run `poetry run pytest --cov=src/transcript_etl_pipeline --cov=scripts/dev_tools --cov-report=term-missing` and confirm tests exit with code 0; if it fails, fix issues and restart from [P4-T1].
  - Acceptance: Command exits with code 0.
- [x] [P4-T5] Run `pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File scripts/dev-tools/run-actionlint.ps1` and confirm it exits with code 0; if it fails, fix issues and restart from [P4-T1].
  - Acceptance: Command exits with code 0.

## Test Plan

- Unit: `poetry run pytest --cov=src/transcript_etl_pipeline --cov=scripts/dev_tools --cov-report=term-missing`
