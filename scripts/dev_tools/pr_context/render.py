...
def extract_plan_sections(plan_text: str) -> tuple[str, str]:
    ...
    verification_block = (
        "Plan verification notes:\n" + truncate(test_plan_section) if test_plan_section else ""
    )
...
def extract_story_parts(user_story_text: str, promoted_story_text: str) -> list[str]:
    ...
    if story_statements:
        story_lines = [line.strip("- ") for line in story_statements.splitlines() if line.strip()]
        if story_lines:
            story_parts.append(
...
def build_pr_context(
    *,
    ...
    gh_available = (
        gh.available if gh_available is None and hasattr(gh, "available") else gh_available
    )
    ...
    verified_closing = current_pr.closing_issues if (current_pr and gh_available) else []
    ...
    try:
        ...
        authors_list = sorted({line.strip() for line in authors.splitlines() if line.strip()})
        ...
        for ref in issue_candidates:
            ...
            entity = gh.classify_entity(number) if gh_available else None
